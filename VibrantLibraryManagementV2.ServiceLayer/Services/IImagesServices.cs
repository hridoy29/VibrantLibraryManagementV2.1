...
namespace VibrantLibraryManagementV2.ServiceLayer.Services
{
    ...
    {
        Task<ImageObject> GetImageUrl(string ImagePath, string token);
    }
    ...
    public class ImagesServices : IImagesServices
    {
        ...
        public async Task<ImageObject> GetImageUrl(string ImagePath, string token)
        {
            try
            {
                ...
                {
                    var ImageObject = await response.Content.ReadFromJsonAsync<ImageObject>();
                    return ImageObject;
                }
